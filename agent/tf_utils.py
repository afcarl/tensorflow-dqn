...
def copy_variable_scope(source_name, target_name):
    ...
    """
    update_target_expr = []
    source_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=source_name)
    target_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=target_name)
